...
describe('CustomPieChart test', () => {
	...
	it('should render wide Icon', () => {
		const wrapper = shallow(<Icon {...props} wide={true} />);
		expect(wrapper.exists()).toBe(true);
	});

	it('should render extra wide Icon', () => {
		const wrapper = shallow(<Icon {...props} xWide={true} />);
		expect(wrapper.exists()).toBe(true);
	});

	it('should render colored Icon', () => {
		const wrapper = shallow(<Icon {...props} color={styles.color.brand.PRIMARY} />);
		expect(wrapper.exists()).toBe(true);
	});
