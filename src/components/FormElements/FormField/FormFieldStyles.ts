...
const StyledFormField = styled.input<{
  ...
  width: ${(props): string =>
    props.small ? "150px" : props.medium ? "210px" : "495px"};
  margin: 8px 0;
  transition: ${styles.transition.PRIMARY};
  :hover {
    border: 1px solid
      ${(props): string =>
        props.error ? styles.color.utility.ERROR : styles.color.shade.NEUTRAL};
    :disabled {
  ...
  :focus {
    ...
    border: 1px solid
      ${(props): string =>
        props.error ? styles.color.utility.ERROR : styles.color.brand.PRIMARY};
    :disabled {
      border: 1px solid ${styles.color.shade.LIGHT};
    }
  }
