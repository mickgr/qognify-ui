...
const StyledCustomRadioButtonLabel = styled.label<{ disabled?: boolean }>`
    ...
    display: flex;
    align-items: center;
    color: ${(props): string => (props.disabled ? styles.color.shade.DARK04 : styles.color.shade.DARK06)};
    cursor: pointer;
...
const StyledCustomRadioButton = styled.input<{ disabled?: boolean }>`
    ...
    :checked + label {
        ...
        :before {
            border: 5px solid
                ${(props): string => (props.disabled ? styles.color.shade.LIGHT : styles.color.brand.PRIMARY)};
        }
    }
`;
