...
describe('icon button tests', () => {
	...
	it('should render default icon button with text', () => {
		const wrapper = shallow(<IconButton {...props} text={<FormattedMessage id={'loading'} />} />);
		expect(wrapper.exists()).toBe(true);
	});
	...
	it('should render default StyledIconButton', () => {
		const wrapper = renderer.create(<StyledIconButton isSecondaryButton={false} text={false} />).toJSON();
		expect(wrapper).toHaveStyleRule('text-decoration', 'none');
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`);
		expect(wrapper).toHaveStyleRule('background-color', `${styles.color.shade.DARK01}`, {modifier: ':hover'});
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`, {modifier: ':hover'});
		expect(wrapper).toHaveStyleRule('background-color', `${styles.color.shade.DARK01}`, {modifier: ':hover:active'});
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`, {modifier: ':hover:active'});
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK03}`, {modifier: ':disabled'});
	});

	it('should render disabled StyledIconButton', () => {
		const wrapper = renderer.create(<StyledIconButton disabled={true} isSecondaryButton={false} text={false} />).toJSON();
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`);
	});

	it('should render secondary StyledIconButton', () => {
		const wrapper = renderer.create(<StyledIconButton isSecondaryButton={true} text={false} />).toJSON();
	...
	it('should render default StyledIconButton with text', () => {
		const wrapper = renderer.create(<StyledIconButton isSecondaryButton={false} text={true} />).toJSON();
		expect(wrapper).toHaveStyleRule('text-decoration', 'none');
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`);
		expect(wrapper).toHaveStyleRule('background-color', `${styles.color.shade.DARK01}`, {modifier: ':hover'});
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`, {modifier: ':hover'});
		...
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`, {modifier: ':hover:active'});
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK03}`, {modifier: ':disabled'});
		expect(wrapper).toHaveStyleRule('font-weight', `${styles.fontWeight.REGULAR}`);
		expect(wrapper).toHaveStyleRule('font-size', `${styles.typographyScale.TYPE16}`);
		expect(wrapper).toHaveStyleRule('line-height', `${styles.lineHeight.LHEIGHT1_5}`);
