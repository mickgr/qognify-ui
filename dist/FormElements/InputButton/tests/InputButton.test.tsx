...
describe('input button tests', () => {
	...
	it('should render disabled input bar', () => {
		const wrapper = renderer.create(<StyledInputButton disabled={true} />).toJSON();
		expect(wrapper).toHaveStyleRule('opacity', '0.6');
		expect(wrapper).toHaveStyleRule('background-color', `${styles.color.shade.DARK03}`);
		expect(wrapper).toHaveStyleRule('background-color', `${styles.color.shade.DARK03}`, {modifier: ':hover'});
	});
