...
describe('form field tests', () => {
	...
	it('should render default StyledFormField', () => {
		...
		expect(wrapper).toHaveStyleRule('border', `1px solid ${styles.color.shade.NEUTRAL}`, {modifier: ':hover'});
		expect(wrapper).toHaveStyleRule('border', `1px solid ${styles.color.brand.PRIMARY}`, {modifier: ':focus'});
	});

	it('should render disabled StyledFormField', () => {
		const wrapper = renderer.create(<StyledFormField disabled={true} />).toJSON();
	...
	it('should render default StyledFormField with error', () => {
		...
		expect(wrapper).toHaveStyleRule('color', `${styles.color.shade.DARK}`);
		expect(wrapper).toHaveStyleRule('border', `1px solid ${styles.color.utility.ERROR}`);
		expect(wrapper).toHaveStyleRule('width', '495px');
		expect(wrapper).toHaveStyleRule('border', `1px solid ${styles.color.utility.ERROR}`, {modifier: ':hover'});
		expect(wrapper).toHaveStyleRule('border', `1px solid ${styles.color.utility.ERROR}`, {modifier: ':focus'});
	});
